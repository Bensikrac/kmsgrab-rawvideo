...
// Helper to find the active framebuffer ID
...
int main() {
    ...
    while (1) {
        // Calculate next frame time (16,666,666 ns = 1/60s)
        ...
        // 3. Check if FB changed or needs initialization
        if (fb_id != last_fb_id) {
            fprintf(stderr, "Framebuffer changed to %u\n", fb_id);
            ...
            if (fb_id == 0) continue; // Skip processing on error
            ...
            image = eglCreateImage(display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, (const EGLAttrib*)img_attribs);
            
            // Close the prime FDs immediately after import (EGL driver holds the reference now)
            for(int i=0; i<num_planes; i++) {
                if (prime_fds[i] >= 0) close(prime_fds[i]);
            }
            ...
            // Resize output buffers if resolution changed
            ...
            if (new_size != size) {
                ...
                fprintf(stderr, "Resolution updated: %dx%d\n", width, height);
            }
        ...
        if (pixels && size > 0) {
            ...
            if (written != size) {
                // Likely broken pipe (consumer stopped)
                break;
            }
        }
    ...
    free(pixels);
    if (image) eglDestroyImage(display, image);
    close(drm_fd);
    return 0;
}
